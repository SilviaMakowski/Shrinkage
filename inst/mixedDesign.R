...
# Sven Hohenstein, Reinhold Kliegl, 2009, 2010, 2011

# Version 0.6.1 (September 2011)
#
# changes to last version v0.6a:
# - the default value for parameter n was changed from '2' to 'ifelse(is.null(W), 2, prod(W))'
...
mixedDesign <- function(B = NULL, W = NULL, n = ifelse(is.null(W), 2, prod(W)), M = 0, SD = 1, R = 0, miss = 0, family = "gaussian", long = FALSE) {
  ...
  if (nB || nW) {
	...
	if (nW) {      
      ...
      for (group in 1 : nBcomb) {      
      	if (!all(dim(as.matrix(R[[group]])) == c(nWcomb, nWcomb))) {
        	...
        	} else if (nB) if (!oneCorMat) print(paste("Using ", R[[group]][1], " as value for all correlations in group ", group, ".", sep =""), quote = FALSE) # not if there is a single matrix/value      			# create correlation matrix        
    ...
    for (group in 1 : nBcomb) {       
      ...
      dat.w[ix, ] <- mvrnorm(n = n, mu = rep(0, nWcomb), Sigma = R[[group]], empirical = TRUE)
    }   
    ...
    if (tolower(family) == "gaussian") {
    	print("...using gaussian distribution", quote = FALSE)
    	...
    	print("...using gamma distribution", quote = FALSE)
    	...
    	print("...using beta distribution", quote = FALSE)
        # generate random beta distributed values
    	for (i in 1:nBcomb) {
      		ix <- (n * (i - 1) + 1) : (n * i)	# subjects within one between-subject factor level combination (lines)
      		for (k in 1:nWcomb)	{
